    ...
    <div class="table-box">
        <div id="toolbar">
            <div class="form-inline">
                ...
                <input type="text" class="form-control" placeholder="请输入领料人..." id="receiver">
                开始时间:
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form>
                    ...
                    <div class="form-group">
                        <label for="recipient-name" class="col-form-label">领料人:</label>
                        <input type="text" class="form-control" id="receiver1" style="width: 550px;">
                    </div>
                    <div class="form-group">
                        <label for="message-text" class="col-form-label">领取数量:</label>
                        <input type="text" class="form-control" style="width: 550px;" id="receiveNum1" onkeyup="this.value=this.value.replace(/\D/g,'')">
                    </div>
                </form>
            </div>
...
<script>
    ...
    $("#delete").click(function () {
        ...
        Ewin.confirm({ message: "确认要删除选择的数据吗？" }).on(function (e) {
            ...
            $.post("/receive/remove",{"ids":ids},function (data) {
                if (data.success) {
    ...
    function add() {
        ...
        if (receiver == null || receiver.trim() == '') {
            toastr.warning("领料人不能为空！");
            return;
        }

        var receiveNum = $("#receiveNum1").val();
        if (receiveNum == null || receiveNum.trim() == '') {
            toastr.warning("领取数量不能为空！");
            return;
        }

        $.post("/receive/add",{"materielNo":materielNo,"receiver":receiver,"receiveNum":receiveNum},function (data) {
            if (data.success) {
                toastr.success(data.responseMsg);
                $('#addModel').modal('hide');
                $("#table").bootstrapTable('refresh');
    ...
    var TableInit = function () {
        ...
        oTableInit.Init = function () {
            $('#table').bootstrapTable({
                url: '/receive/listReceiveMateriel',     //请求后台的URL（*）
                method: 'get',           //请求方式（*）
